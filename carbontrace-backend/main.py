...
_ROOT_DIR = _pl.Path(__file__).parent.parent.resolve()
_CSV_DIR = str(_ROOT_DIR / "dataset_csv")
...
UPLOAD_DIR = os.path.join(str(_ROOT_DIR), "uploads")
...
def get_report_summary(nse_code: str = Query(default=""), db: Session = Depends(get_db)):
    ...
    try:
        nar_df = pd.read_csv(str(_ROOT_DIR / "dataset_csv" / "narrative_train.csv"))
...
def get_narrative_samples(n: int = Query(default=5), db: Session = Depends(get_db)):
    ...
    df = pd.read_csv(str(_ROOT_DIR / "dataset_csv" / "narrative_train.csv"))
